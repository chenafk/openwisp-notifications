...
function getObjectNotificationComponent() {
    ...
    <li class="ow-object-notification-container">
        <a href="#" id="ow-object-notify" class="ow-object-notify" title="You are receiving notifications for this object.">
            ...
            <span id="ow-silence-label">${gettext('Silence notifications')}</span>
        </a>
        <div class="ow-object-notification-option-container option-container ow-hide">
            <p id="ow-notification-help-text">${gettext('Disable notifications for this object')}</p>
            ...
            <button data-days=1 class="ow-notification-option option disable-notification">${gettext('1 Day')}</button>
            <button data-days=7 class="ow-notification-option option disable-notification">${gettext('1 Week')}</button>
            <button data-days=30 class="ow-notification-option option disable-notification">${gettext('1 Month')}</button>
            <button data-days=-1 class="ow-notification-option option disable-notification">${gettext('Permanently')}</button>
    ...
    $(document).on('click', '#ow-enable-notification', function (e) {
        ...
        $.ajax({
            ...
            success: function () {
                ...
                $('#ow-silence-label').html(gettext('Silence notifications'));
                $('#ow-object-notify').prop('title', 'You are receiving notifications for this object.');

                $('#ow-notification-help-text').html(gettext(`Disable notifications for this object`));
                $('#ow-object-notification-loader').addClass('ow-hide');
...
function updateObjectNotificationHelpText($, validTill) {
    ...
    $('#ow-silence-label').html(gettext('Unsilence notifications'));
